...
```{ojs}
html`<div name="file-import">${viewof csvfile2}</div>`
```
...
```{ojs}
viewof csvfile2 = Inputs.file({label: md`__Fichier à annoter__ 📁 ([Exemple ici](https://raw.githubusercontent.com/linogaliana/serverless-annotation-tool/master/example2.csv))`, accept: ".csv", required: true})
```
