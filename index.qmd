...
viewof text_input = (count,
  Inputs.text({label: html`<b>Raison de la décision</b>`, placeholder: "Un texte explicatif"})
)
```
...
```{ojs}
share = Math.round(100*current_row/Number(n_rows))
html`<div style="background:green;width:${share}%;height:12px;">`
```


```{ojs}
//| echo: false
viewof download = DOM.download(serialize(x), null, "Sauvegarder les annotations 👇 dans un fichier")
```
